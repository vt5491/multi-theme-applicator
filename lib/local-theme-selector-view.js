  ...
  (LocalThemeSelectorView = (function() {
    LocalThemeSelectorView = class LocalThemeSelectorView {
      ...
      initThemeSelectorForm() {
        ...
        //vt add
        // following key is useful during development.
        // Comment out otherwise.
        // console.log("ltsv: about to add debug btn");
        // const $debugBtn = $('<button type="button"></button>');
        // $debugBtn.text('Debug');
        // $debugBtn.attr('id', 'debug');
        // $debugBtn.appendTo($submitDiv);
        // return $debugBtn.click(() => {
        //   debugger;
      ...
      applyLocalTheme(fn, themePath, scope, ed) {
        //vt add
        console.log("vt.applyLocalTheme: fn=" + fn + ",themePath=" + themePath + ",scope=" + scope + ",ed=" + ed);
        // debugger;
        //vt end
        ...
        console.log("vt.ltsv.applyLocalTheme: targetFile=" + targetFile);
        if (!targetFile) {
...
Please open a file and then apply a local theme.`;
        ...
        return promise
          .then(
            result => {
              ...
              console.log("vt.ltsv.applyLocalTheme.promise: targetFile=" + targetFile + ",themeScope=" + themeScope);
              switch (themeScope) {
                case "fileType": case "file": case "editor":
                  ...
                  console.log("vt.ltsv.applyLocalTheme.promise: targetFile=" + targetFile + ",editors=" + editors);
                  for (let editor of Array.from(editors)) {
                    ...
                    //vt add
                    console.log("vt.ltsv.applyLocalTheme: editor.title=" + editor.getTitle() + ", editor.path=" + editor.getPath());
                    // for unsaved files, we need to know when they're initially saved so we can
                    // style by the fn, not the dummy one (.e.g "untitled-8").
                    if (!editor.getPath()) {
                      // editor.onDidSave = this.utils.editorInitSaved;
                      let key = editor.getTitle() + "-" + editor.id;
                      // create a closure with key set, so we can dispose of the hook upon first call.
                      // let disposble = editor.onDidSave(this.utils.editorInitSavedByKey(key));
                      // base.Disposables[key] = disposable;
                      // Base.Disposables[key] = editor.onDidSave(this.utils.editorInitSavedByKey(key));
                      Base.Disposables[key] = editor.onDidSave(this.editorInitSavedByKey(key));
                    }
                    //vt end
      ...
      //vt add
      // this.addEventListener('abc', function (e) { console.log("hi from abc") }, false);
      // this method is called when an 'untitled' editor first gets saved.  We want to convert
      // the fileLookup to be by filename instead of 'untitled-id'.
      editorInitSaved(key) {
        // console.log("ltsv.editorInitSaved: e=" + e + ", e.path=" + e.path + ",key=" + key);
        return (e => {
          console.log("ltsv.editorInitSaved: e.path=" + e.path + ",key=" + key)
          // console.log("ltsv.fileLookup=" + this.fileLookup);
          console.log("ltsv.fileLookup: %o", this.fileLookup);
          // debugger;
          // console.log("ltsv.editorInitSave: Base.Disposables.pre.pre=" + Base.Disposables);
          Base.Disposables[key].dispose();
          console.log("ltsv.editorInitSave: Base.Disposables.keys.pre=" + Object.keys(Base.Disposables).length);
          delete Base.Disposables[key];
          console.log("ltsv.editorInitSave: Base.Disposables.keys.post=" + Object.keys(Base.Disposables).length);
          ...
          // debugger;
          let currentStyle = this.fileLookup[key];
          console.log("editorInitSave: current style=" + currentStyle);
          let fn_path = String(e.path.replace(/\\/g, '/'));
          console.log("editorInitSave: fn_path=" + fn_path);
          this.fileLookup[fn_path] = currentStyle;
          delete this.fileLookup[key];
          console.log("ltsv.new fileLookup: %o", this.fileLookup);
        })
      }

      editorInitSavedByKey(key){
        console.log("ltsv.editorInitSavedByKey:key=" + key);
